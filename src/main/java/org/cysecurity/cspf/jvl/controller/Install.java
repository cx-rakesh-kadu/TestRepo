                      ...
                      if(con!=null && !con.isClosed())
                        {
                              ...
                              if(!con.isClosed())
                            {
                                ...
                                stmt.executeUpdate("Create table users(ID int NOT NULL AUTO_INCREMENT, username varchar(30),email varchar(60), password varchar(60), about varchar(50),privilege varchar(20),avatar TEXT,secretquestion int,secret varchar(30),primary key (id))");
                                  // Use PreparedStatement to prevent SQL injection for user-supplied adminuser/adminpass
                                  PreparedStatement adminStmt = con.prepareStatement("INSERT into users(username, password, email,About,avatar, privilege,secretquestion,secret) values (?,?,'admin@localhost','I am the admin of this application','default.jpg','admin',1,'rocky')");
                                  adminStmt.setString(1, adminuser);
                                  adminStmt.setString(2, adminpass);
